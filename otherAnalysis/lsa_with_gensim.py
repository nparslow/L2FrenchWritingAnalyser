...
class FileAnalyser(object):
    ...
    def analyseTarFile( self, filename, sentences):
        with tarfile.open(filename, mode='r') as tar:
            ...
            for tarinfo in tar:
                ...
                if re.match(r"^frwikipedia_\w+\.E\w+\.dis\.dep\.xml$", basefilename):
                    ...
                    with tar.extractfile(filename) as fileobject:
                        self.analyseFileWiki(fileobject, sentences)

    def analyseDirectory( self, inbasepath, sentences):
        # it's a directory:
        for element in os.listdir(inbasepath ):
            ...
            if os.path.isfile(in_full_element) and tarfile.is_tarfile(in_full_element):
                self.analyseTarFile(in_full_element, sentences )
            elif os.path.isdir(in_full_element):
                # analyse the directory
                self.analyseDirectory(in_full_element, sentences )


    def analyseFileWiki( self, fileobject, sentences ):
...
fileanalyser = FileAnalyser(sentences)
fileanalyser.analyseDirectory(inbasepath, sentences)
